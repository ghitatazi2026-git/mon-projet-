...
> ⚠️ **Note :** Ce projet est conçu à des fins éducatives et de démonstration SOC (Security Operations Center).
...
* 🛡️ **Active Defense (Blocage IP)** : Détection d'IPs suspectes et bouton "BAN IP" permettant d'exécuter une règle `iptables` sur la machine distante.
...
python app.py
```
Le tableau de bord sera ensuite accessible via votre navigateur à l'adresse : **[http://localhost:5000](http://localhost:5000)**.
